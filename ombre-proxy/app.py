...
def get_session():
    ...
    if OMBRE_PASSWORD:
        try:
            login_resp = requests.post(f"{OMBRE_URL}/api/login", json={"password": OMBRE_PASSWORD}, timeout=10)
...
def mcp_call(tool_name, arguments=None):
    ...
    headers = {"Content-Type": "application/json", "Cookie": f"ombre_session={session}"}
    try:
        resp = requests.post(f"{OMBRE_URL}/mcp", json=payload, headers=headers, timeout=15)
        if resp.status_code == 401 or resp.status_code == 403:
            clear_session()
            return None
        if resp.status_code == 404:
            resp2 = requests.post(f"{OMBRE_URL}/", json=payload, headers=headers, timeout=15)
...
def chat():
    ...
    if memory:
        ...
        for msg in messages:
            if msg["role"] == "system":
                msg["content"] = f"{msg['content']}\n\n[以下是你过去的记忆。自然地参考它们，不要逐条复述。]\n{memory}"
                has_system = True
                break
        if not has_system:
            messages.insert(0, {"role": "system", "content": f"[以下是你过去的记忆。自然地参考它们，不要逐条复述。]\n{memory}"})
